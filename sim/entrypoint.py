#!/usr/bin/env python3
"""fika sim container entrypoint.

Turns environment variables into a running simulated espresso machine:

  1. Copies the repo's esphome/ configs into the cache volume and writes
     secrets.yaml from MQTT_* env vars (the broker hostname is resolved to
     an IP first; QEMU user-mode networking cannot use container-only names
     like host.containers.internal).
  2. Compiles esphome/sim-fika.yaml, the REAL firmware, esp32 target.
...
Environment:
  ...
  MQTT_ROOT=fika, SIM_NODE=sim-fika, SIM_TIMEZONE=Europe/Stockholm
...
def main() -> int:
    ...
    node = env("SIM_NODE", "sim-fika")
    timezone = env("SIM_TIMEZONE", "Europe/Stockholm")
    print(f"fika sim: node={node} broker={broker} ({broker_ip}) "
    ...
    print(f"fika sim: {exited} exited with {code}; stopping", flush=True)
    shutdown()
    for p in procs.values():
        try:
            p.wait(timeout=10)
