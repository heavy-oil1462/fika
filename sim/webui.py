#!/usr/bin/env python3
"""fika simulator web UI: drive the REAL firmware's inputs by hand.

Serves a small control panel (stdlib http.server, no dependencies beyond
paho-mqtt) that lets you set every simulated espresso machine input
individually and watch the firmware react:
  ...
  * Sensor injections, published retained to <root>/<node>/sim/<key>, where
    ...
    up under the real sensor ids. The boiler PID / cup recognition / brew
    lambdas that respond are the exact C++ flashed onto real nodes.
  * Time of day, served to the firmware over SNTP. sim-fika.yaml points
    ${sntp_server} at this process (10.0.2.2 from inside QEMU), so the
    firmware's clock follows the slider. Requires --ntp-port 123
    (privileged; fine inside the container).
  * Live entity state: everything the node publishes, straight from the
    broker, so you can see heater/pump/valve/status respond within seconds.

Runs inside the sim container (see sim/Containerfile) but works equally well
straight from a checkout against any broker:

    python3 sim/webui.py --broker 192.168.1.10 --username fika \
...
# (tools/validate.py sim cross-checks them). Switches are 0/1 floats:
...
class SimBridge:
    ...
    def __init__(self, args):
        ...
        self._lock = threading.Lock()
        self.client = mqtt_client(f"fika-sim-ui-{os.getpid()}",
...
PAGE = """<!doctype html>
<html><head><meta charset="utf-8"><title>fika simulator</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
 body{font-family:system-ui,sans-serif;margin:0;background:#1a1410;color:#e8dfd6}
 header{padding:.8rem 1.2rem;background:#251d16;border-bottom:1px solid #423528}
 h1{font-size:1.1rem;margin:0}h1 small{color:#a38b6f;font-weight:normal}
 main{display:grid;grid-template-columns:minmax(320px,430px) 1fr;gap:1rem;padding:1rem}
 @media(max-width:860px){main{grid-template-columns:1fr}}
 section{background:#251d16;border:1px solid #423528;border-radius:8px;padding:1rem}
 h2{font-size:.85rem;text-transform:uppercase;letter-spacing:.08em;color:#a38b6f;margin:0 0 .8rem}
 .row{margin-bottom:.9rem}
 .row label{display:flex;justify-content:space-between;font-size:.85rem;margin-bottom:.2rem}
 .row output{color:#d4b89f;font-variant-numeric:tabular-nums}
 input[type=range]{width:100%;accent-color:#af7a4c}
 button{background:#402f21;color:#e8dfd6;border:1px solid #5f4a34;border-radius:6px;
        padding:.35rem .7rem;margin:.15rem;cursor:pointer;font-size:.8rem}
 button:hover{background:#523d2a}
 table{width:100%;border-collapse:collapse;font-size:.82rem}
 td,th{padding:.25rem .4rem;border-bottom:1px solid #382c20;text-align:left}
 td.v{color:#d4b89f;font-variant-numeric:tabular-nums}
 td.age{color:#7a6a55;font-size:.75rem;white-space:nowrap}
 .pill{display:inline-block;padding:.1rem .55rem;border-radius:999px;font-size:.75rem}
 .on{background:#4d3a1d;color:#d4b89f}.off{background:#4d1d1d;color:#d4a09f}
 .muted{color:#7a6a55;font-size:.78rem}
 #timeview{font-size:1.3rem;color:#d4b89f;font-variant-numeric:tabular-nums}
</style></head><body>
<header><h1>fika simulator <small id="conn">connecting…</small></h1></header>
<main>
<div>
<section><h2>Sensor injections</h2><div id="sliders"></div>
 ...
 sensors adopt them under the real sensor ids. Switches are 0/1.</p></section>
<section><h2>Time of day</h2>
 ...
 <p class="muted">Served to the firmware over SNTP; the on-device clock
 follows this slider.</p></section>
...
def main() -> int:
    ...
    ap.add_argument("--root", default=os.environ.get("MQTT_ROOT", "fika"))
    ap.add_argument("--node", default=os.environ.get("SIM_NODE", "sim-fika"))
    ...
    print(f"fika simulator UI: http://0.0.0.0:{args.http_port} "
